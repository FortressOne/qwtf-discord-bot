...
class QwtfDiscordBotPug # :nodoc:
  ...
  def run
    ...
    bot.register_application_command(:foo, "Foo", server_id: "744911007859736616") do |cmd|
      cmd.subcommand_group(:bar, "Bar") do |group|
        group.subcommand("baz", "this is the Baz subcommand") do |sub|
          sub.string("qux", "Ask a question", required: true)
        end
      end
    end

    bot.register_application_command(:spongecase, 'Are you mocking me?', server_id: "744911007859736616") do |cmd|
      cmd.string('message', 'Message to spongecase')
      cmd.boolean('with_picture', 'Show the mocking sponge?')
    end

    bot.application_command(:foo).group(:bar) do |group|
      group.subcommand("baz") do |event|
        event.respond(content: "Baz response", ephemeral: true)
      end
    end
    ...
    bot.register_application_command(:help, 'Get help', server_id: "744911007859736616")

    bot.application_command(:help) do |event|
      embed = Discordrb::Webhooks::Embed.new
      embed.description = HELP[:commands]
      event.respond(embeds: [embed.to_hash]) # due to bug in discordrb, will be fixed
    end

    bot.register_application_command(:join, 'Join the PUG queue', server_id: "744911007859736616") do |cmd|
      cmd.string('users', 'Add someone else to the PUG')
    end

    bot.application_command(:join) do |event|
      setup_pug(event) do |e, pug|
        if !event.options["users"]
        ...
        event.options["users"].scan(VALID_MENTION).each do |mention|
        ...
        if pug.total_player_count == 0
          ...
          descriptions << "#{e.display_name} creates a PUG"
        ...
        embed = Discordrb::Webhooks::Embed.new
        embed.description = [errors, descriptions.join(MSG_SNIPPET_DELIMITER)].join("\n")
        event.respond(content: message, embeds: [embed.to_hash]) # due to bug in discordrb, will be fixed

        start_pug(pug, e) if pug.has_exactly_maxplayers?
      end
    else
      event.respond(content: "NO!")
    end
  end
